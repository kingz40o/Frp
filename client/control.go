...
func (ctl *Control) connectServer() (conn net.Conn, err error) {
	...
	if ctl.clientCfg.TCPMux {
		...
		if protocol == "websocket" {
			protocol = "tcp"
			dialOptions = append(dialOptions, libdial.WithAfterHook(libdial.AfterHook{Hook: frpNet.DialHookWebsocket(ctl.clientCfg.WebSocketPath, ctl.clientCfg.WebSocketHost)}))
		}
